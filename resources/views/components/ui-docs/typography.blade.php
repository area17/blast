    ...
    <div {{ $attributes }}>
        @foreach ($items as $key => $item)
            <div class="{{ $loop->index > 0 ? 'blast:mt-4 blast:pt-4 blast:border-t' : '' }} blast:border-solid blast:border-gray-200">
                <div
                    class="blast:mb-2 blast:p-2 blast:border-2 blast:border-solid blast:border-gray-200 blast:bg-gray-100 blast:whitespace-nowrap blast:text-ellipsis blast:overflow-hidden"
                    style="{{ $property }}: {{ $item }};"
                >
